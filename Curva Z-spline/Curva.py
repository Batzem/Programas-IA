...
#----------------------------------------------
# ESFM IPN
# TONATIUH HERNANDEZ 5AV1 FUNDAMENTOS DE IA
#-----------------------------------------------
import numpy as np
...
class curva:
  ...
  """
    Curva que construye la curva que pasa por los puntos x
  ...
  def lista_de_puntos(s) -> str:
      ...
      def interpolacion(s,p:int=0, r:float=0.0) -> list:
        ...
        if p == 0:
            ...
            if i ==s.n-1
                ip1 = 0
            xi.appnd(a*s.x[ip1] + (1.0-a) * s.x[i])
            for j in range(1,s.dim):
        ...
        #--------------------------------
          ...
          xi.append(zp1*s.x[ip1]+z*s.x[i]+zp2*s.x[ip2+j*s.n]+ am1*s.x[im1+j*s.n])
          for j in range(1,s.dim):
